...
const useResponsiveDimension = (responsive: boolean, config: GradualBlurConfig, key: 'height' | 'width') => {
  ...
  useEffect(() => {
    ...
    const calc = () => {
      ...
      let v = config[key];
      const capitalKey = key[0].toUpperCase() + key.slice(1);
      
      if (w <= 480 && (config as any)['mobile' + capitalKey])
        v = (config as any)['mobile' + capitalKey];
      else if (w <= 768 && (config as any)['tablet' + capitalKey])
        v = (config as any)['tablet' + capitalKey];
      else if (w <= 1024 && (config as any)['desktop' + capitalKey])
        v = (config as any)['desktop' + capitalKey];
      setVal(v);
    };
...
const GradualBlur: React.FC<GradualBlurConfig> = (props) => {
